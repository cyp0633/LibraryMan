...
public:
    ...
    void borrowBook(repo &r) //借书功能
    {
        ...
        case 3:
            ...
            break;
        case 2:
            std::cout << "请输入您想查找的ISBN/ISSN，记得输入中间的连字符:";
            cin >> target;
        ...
        if (r.bookList[bookNum].borrow()) //书被成功借走，会返回true，这样才会计入借走的列表
        {
            ...
            std::cout << "您借走的书馆内编号为" << bookNum << "，请妥善保管此数字，还书时将用到。";
...
account::void modifyData()
{
    return;
}
void fileImportAccount()
{
    std::cout << "即将从account.txt读取账户信息,请确认没有重复的账号。\n请注意：密码格式为字母开头，可以包含字母/数字/下划线，6-17位。\n如果不符合要求，将不会导入。";
    string username;
    string password;
    int count = 0;
    regex passFormat("^[a-zA-Z][a-zA-Z0-9_]{5,17}$");
